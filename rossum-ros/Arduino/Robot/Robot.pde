...
/*  
  v = _SpeedController.CommandedVelocity;
  o = _SpeedController.CommandedAngularVelocity;
*/
 ...
 /*
  if(o < 0)
  {
    normalizedRightMotorCV = v + o;
    normalizedLeftMotorCV = v;
  }
  else
  {
    normalizedRightMotorCV = v;
    normalizedLeftMotorCV = v - o;
  }
  */
...
//  float leftServoValue = mapFloat(normalizedLeftMotorCV, -1, 1, 90.0 - c_MaxMotorCV, 90.0 + c_MaxMotorCV);     // scale it to use it with the servo (value between 0 and 180) 
  float rightServoValue = mapFloat(normalizedRightMotorCV, -0.5, 0.5, -255, 255);     // scale it to use it with the servo (value between 0 and 180) 
  float leftServoValue = mapFloat(normalizedLeftMotorCV, -0.5, 0.5, -255, 255);     // scale it to use it with the servo (value between 0 and 180) 
...
//  _LeftServo.write(leftServoValue);     // sets the servo position according to the scaled value (0 ... 179)
  ...
  if(leftServoValue >= 0 && rightServoValue >= 0)
  {
  ...
  analogWrite(speedPinA, abs(rightServoValue));
  analogWrite(speedPinB, abs(leftServoValue)); 
