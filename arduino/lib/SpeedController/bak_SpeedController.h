...
private:
	...
	{
		if (integralContribution > 0.5)
		{
			return 0.5;
		}
		else if (integralContribution < -0.5)
		{
			return -0.5;
		}
	}
...
/*
		if(angularVelocityOffset < 0.02 && angularVelocityOffset > -0.02)
	{	
	...
	*/
		...
		bool expectedLeftSpeedLessZero = false;
		bool expectedRightSpeedLessZero = false;

		if(expectedLeftSpeed < 0)
                {
			expectedLeftSpeed = -expectedLeftSpeed;
			expectedLeftSpeedLessZero = true;
		}
		if(expectedRightSpeed < 0)
		{
			expectedRightSpeed = -expectedRightSpeed;
			expectedRightSpeedLessZero = true;
		}

        LeftError = expectedLeftSpeed - abs(_pOdometricLocalizer->VLeft);
		RightError = expectedRightSpeed - abs(_pOdometricLocalizer->VRight);
		...
		if(expectedLeftSpeedLessZero)
		{
			NormalizedLeftCV = -NormalizedLeftCV;
		}
		if(expectedRightSpeedLessZero)
		{
